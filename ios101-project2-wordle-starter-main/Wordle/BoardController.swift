...
class BoardController: NSObject,
  ...
  private func applyThemeSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      if let theme = settings[kWordThemeKey] as? String{
          if let themeWord = WordTheme(rawValue:theme){
              goalWord = WordGenerator.generateGoalWord(with: themeWord);
          }
      }
  ...
  private func applyIsAlienWordleSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      if let changegoal  = settings[kIsAlienWordleKey] as? Bool{
          isAlienWordle = changegoal
    }
    // START YOUR CODE HERE
  }
}
